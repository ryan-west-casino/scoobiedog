...
class OaksMain extends GameKernel
{
    use GameKernelTrait;
    ...
    */
    public function game_launch($game_content) {
        return view('wainwright::launcher-content-oaks')->with('game_content', $game_content);
    }
    ...
    */
    public function modify_game($token_internal, $game_content)
    {
        ...
        $new_api_endpoint = config('casino-dog.games.3oaks.new_api_endpoint').$token_internal.'/';  // building up the api endpoint we want to receive game events upon
        ...
        $gc = str_replace('//betman-demo.head.3oaks.com/betman-demo/gs/', $new_api_endpoint.$select_session['currency'].'/play?origin_url=https://betman-demo.head.3oaks.com/betman-demo/gs/', $gc);  // swap the legitimate game endpoint to ours
