...
class WainwrightMain extends GameKernel
{
    use GameKernelTrait;
    ...
    */
    public function load_game_session($data) {
        ...
        $session = new WainwrightSessions();
    ...
    */
    public function game_launch($game_content) {
        return view('wainwright::launcher-content-wainwright')->with('game_content', $game_content);
    }
    ...
    public function dynamic_asset(string $asset_name, Request $request) 
    {
        /* example *
            ...
            if($asset_name === 'minilobby.json') {
                ...
                $gameStartURL = config('gameconfig.wainwright.minilobby_url').'/'.$signature.'/'.$time;
    ...
    public function fake_iframe_url(string $slug, $currency) {
        /* example *
            ...
            if($currency === 'DEMO' || $currency === 'FUN') {
                $build_url = 'https://bog.wainwright.net/gs2c/openGame.do?gameSymbol='.$game_id.'&websiteUrl=https%3A%2F%2Fblueoceangaming.com&platform=WEB&jurisdiction=99&lang=en&cur='.$currency;
            }
            $build_url = 'https://bog.wainwright.net/gs2c/html5Game.do?gameSymbol='.$game_id.'&websiteUrl=https%3A%2F%2Fblueoceangaming.com&platform=WEB&jurisdiction=99&lang=en&cur='.$currency;
            return $build_url;
    ...
    */
    public function modify_game($token_internal, $game_content)
    {
        ...
        $new_api_endpoint = config('casino-dog.games.wainwright.new_api_endpoint').$token_internal.'/'.$select_session['game_id_original'].'/play';  // building up the api endpoint we want to receive game events upon
