...
class HacksawGame extends HacksawMain
{
    ...
    public function game_event($request)
    {
        ...
        $url = str_replace('games/hacksaw/'.$internal_token.'/'.$select_session['game_id_original'].'/play', '', $request->fullUrl());
        $url = str_replace('scoobiedog.casinoman.app/api', 'rgs-demo.hacksawgaming.com/api', $url);
