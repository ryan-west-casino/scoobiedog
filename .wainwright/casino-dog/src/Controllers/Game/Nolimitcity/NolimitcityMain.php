...
class NolimitcityMain extends GameKernel
{
    use GameKernelTrait;
    ...
    */
    public function load_game_session($data) {
        ...
        $session = new NolimitcitySessions();
    ...
    */
    public function game_launch($game_content) {
        return view('wainwright::launcher-content-nolimitcity')->with('game_content', $game_content);
    }
    ...
    public function dynamic_asset(string $asset_name, Request $request) {
        if($asset_name === 'nolimit-js.js') {
            ...
            $data = [
                ...
                'signature' => hash_hmac('sha256', $token, env('PUSHER_APP_KEY')),
                'token' => $token
            ];

            return view('wainwright::scripts.nolimit-js')->with('data', $data);
        }
    }
    ...
    */
    public function modify_game($token_internal, $game_content)
    {
        ...
        $gc = $game_content;
        $gc = str_replace('https://nolimitjs.nolimitcdn.com/dist/nolimit-latest.js', 'https://'.$token_internal.config('casino-dog.wildcard_session_domain.domain').'/dynamic_asset/nolimitcity/nolimit-js.js?token='.$token_internal, $game_content);
