...
class PlayngoSessions extends PlayngoMain
{
    ...
    public function fresh_game_session($game_id, $method, $token_internal = NULL)
    {
        if($method === 'demo_method') {
            ...
            $url = 'https://asccw.playngonetwork.com/Casino/IframedView?pid=2&gid='.$game_id.'&lang=en_US&practice=1&channel=desktop&div=flashobject&width=100%25&height=100%25&user=&password=&ctx=&demo=2&brand=&lobby=&rccurrentsessiontime=0&rcintervaltime=0&rcaccounthistoryurl=&rccontinueurl=&rcexiturl=&rchistoryurlmode=&autoplaylimits=0&autoplayreset=0&callback=flashCallback&rcmga=&resourcelevel=0&hasjackpots=False&country=&pauseplay=&playlimit=&selftest=&sessiontime=&coreweburl=https://asccw.playngonetwork.com/&showpoweredby=True';
           // $url = 'https://bmtcw.playngonetwork.com/casino/ContainerLauncher?lang=en_GB&origin=https%3A%2F%2Fenergycasino.com%2F&gid='.$game_id.'&practice=1&pid=2&channel=desktop';
    ...
    public function create_session(string $internal_token)
    {
        ...
        $changed_content = $this->modify_game($token_internal, $game, $game['gid']);
