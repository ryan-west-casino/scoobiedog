<?php
namespace Wainwright\CasinoDog\Controllers\Game\Habanero;

use Wainwright\CasinoDog\Controllers\Game\GameKernel;
use Wainwright\CasinoDog\Controllers\Game\GameKernelTrait;
use Illuminate\Http\Request;

class HabaneroMain extends GameKernel
{
    use GameKernelTrait;
    ...
    */
    public function game_launch($game_content) {
        return view('wainwright::launcher-content-habanero')->with('game_content', $game_content);
    }
    ...
    public function dynamic_asset(string $asset_name, Request $request) 
    {
        /* example *
            ...
            if($asset_name === 'minilobby.json') {
                ...
                $gameStartURL = config('gameconfig.habanero.minilobby_url').'/'.$signature.'/'.$time;
    ...
    public function fake_iframe_url(string $slug, $currency) {
        /* example *
            ...
            if($currency === 'DEMO' || $currency === 'FUN') {
                $build_url = 'https://bog.habanero.net/gs2c/openGame.do?gameSymbol='.$game_id.'&websiteUrl=https%3A%2F%2Fblueoceangaming.com&platform=WEB&jurisdiction=99&lang=en&cur='.$currency;
            }
            $build_url = 'https://bog.habanero.net/gs2c/html5Game.do?gameSymbol='.$game_id.'&websiteUrl=https%3A%2F%2Fblueoceangaming.com&platform=WEB&jurisdiction=99&lang=en&cur='.$currency;
            return $build_url;
    ...
    */
    public function modify_game($token_internal, $game_content)
    {
        ...
        $new_api_endpoint = config('casino-dog.games.habanero.new_api_endpoint').$token_internal.'/'.$select_session['game_id_original'].'/play';  // building up the api endpoint we want to receive game events upon
        ...
        */
        $gc = str_replace('https://gi-e.insvr.com/pf', $new_api_endpoint, $gc);
        $gc = str_replace('\"IsFunMode\":true', '\"IsFunMode\":false', $gc);
        $gc = str_replace('\"CurCode\":\"ZZZ', '\"CurCode\":\"USD', $gc);
        $gc = str_replace('\"CurSymbol\":\"', '\"CurSymbol\":\"\$\", \"CurOlderSymbol\":\"', $gc);
