...
class HabaneroGame extends HabaneroMain
{
    use GameKernelTrait;

    public function game_event($request)
    {
        $real_gameserver_url = 'https://gi-e.insvr.com/pf';
        ...
        if(isset($data_origin['info'])) {
           $data_origin['info']['message'] = "WELCOME\n\nYou are playing Wainwright slotmachine.";
        }

        if(isset($data_origin['header']['player'])) {

        $origin_balance = Cache::get($internal_token.'::habaneroOriginBalance');
        if(!$origin_balance) {
            Cache::set($internal_token.'::habaneroOriginBalance', $data_origin['header']['player']['displaybalance']);
            $origin_balance = Cache::get($internal_token.'::habaneroOriginBalance');
        }
        $new_balance = $data_origin['header']['player']['displaybalance'];
        ...
        if($origin_balance !== $new_balance) {
            ...
            $data_origin['header']['player']['displaybalance'] = floatval(($process_and_get_balance / 100));
        }
        }
