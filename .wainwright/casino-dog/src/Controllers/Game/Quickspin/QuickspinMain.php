...
class QuickspinMain extends GameKernel
{
    use GameKernelTrait;
    ...
    */
    public function game_launch($game_content) {
        return view('wainwright::launcher-content-quickspin')->with('game_content', $game_content);
    }
    ...
    */
    ...
    {
        if($asset_name === 'rlx-launcher.js') {
            return $this->pretendResponseIsFile(__DIR__.'/AssetStorage/relax-launcher.js', 'application/javascript; charset=utf-8');
        }
        if($asset_name === 'relax.js') {
            return $this->pretendResponseIsFile(__DIR__.'/AssetStorage/relax.js', 'application/javascript; charset=utf-8');
        }
        if($asset_name === 'current-device.min.js') {
            return $this->pretendResponseIsFile(__DIR__.'/AssetStorage/current-device.min.js', 'application/javascript; charset=utf-8');
        }
        if($asset_name === 'launchermain.js') {
            return $this->pretendResponseIsFile(__DIR__.'/AssetStorage/launcher-main.js', 'application/javascript; charset=utf-8');
        }
        if($asset_name === 'loadscript.js') {
            return $this->pretendResponseIsFile(__DIR__.'/AssetStorage/loadscript.js', 'application/javascript; charset=utf-8');
        }
        if($asset_name === 'QueryData.compressed.js') {
            return $this->pretendResponseIsFile(__DIR__.'/AssetStorage/querydata.js', 'application/javascript; charset=utf-8');
        }
        ...
        /* example *
            ...
            if($asset_name === 'minilobby.json') {
                ...
                $gameStartURL = config('gameconfig.quickspin.minilobby_url').'/'.$signature.'/'.$time;
    ...
    public function fake_iframe_url(string $slug, $currency) {
        /* example *
            ...
            if($currency === 'DEMO' || $currency === 'FUN') {
                $build_url = 'https://bog.quickspin.net/gs2c/openGame.do?gameSymbol='.$game_id.'&websiteUrl=https%3A%2F%2Fblueoceangaming.com&platform=WEB&jurisdiction=99&lang=en&cur='.$currency;
            }
            $build_url = 'https://bog.quickspin.net/gs2c/html5Game.do?gameSymbol='.$game_id.'&websiteUrl=https%3A%2F%2Fblueoceangaming.com&platform=WEB&jurisdiction=99&lang=en&cur='.$currency;
            return $build_url;
    ...
    */
    public function modify_game($token_internal, $game_content)
    {
        ...
        $new_api_endpoint = config('casino-dog.games.quickspin.new_api_endpoint').$token_internal.'/'.$select_session['game_id_original'].'/play';  // building up the api endpoint we want to receive game events upon

        $gc = $game_content;
        
       //$gc = str_replace('t" src="', 't" src="https://wainwrighted.herokuapp.com/https://d10zgitni74b5t.cloudfront.net/casino/', $gc);
        $gc = str_replace('launcherlibs', 'dynamic_asset/quickspin', $gc);
