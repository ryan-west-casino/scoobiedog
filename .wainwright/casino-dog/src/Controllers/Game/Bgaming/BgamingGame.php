...
class BgamingGame extends BgamingMain
{
    public static function proxy_event($internal_token, $request) {
        if(str_contains($request->path(), '/games/wainwright')) {
            $resp = ProxyHelperFacade::CreateProxy($request)->toHost('https://bgaming-network.com/api', 'api/games/wainwright/'.$internal_token);
        } else {
                ...
                if(isset($data_origin['api_version'])) {
                    ...
                    // "Commands" below are on player side/game client.
                        if($request->command === 'init' || $request->command === 'close') {
                            ...
                            if(isset($data_origin['outcome']['freespins_issued_DISABLED'])) // <<<DISABLED, remove DISABLED to load respin >>
